...
saveFig = True
saveFigAs \
  = '/home/kkadoogan/fig.AdibaticCollapse_XCFC_MultiPanel_PostBounce.png'

# Rs/km                 100 , 500 , 1000, 2000, 3000
snapshots = np.array( [ 2522, 2630, 2840, 3408, 4600 ], dtype = np.int64 )
...
for iSS in range( snapshots.shape[0] ):
    ...
    if plotFields == 'hydro':
        ...
        axs[1,1].semilogy( X1, T, c = color[iSS], \
                           label = r'$t-t_{{\mathrm{{b}}}}={:.1f}\ \mathrm{{ms}}$' \
                                   .format( Time - tb )  )
        ...
        Ye, DataUnit, \
          = GetData( plotfileDirectory, plotfileBaseName, 'AF_Ye', \
                     ...
                     ReturnTime = False, ReturnMesh = False, Verbose = False )
        Ye = np.copy( Ye[:,0,0] )
        axs[1,0].plot( X1, Ye, ls = '-', c = color[iSS] )
